 ...
 * @author Luckbliss
 */
public class KPAThreeTab extends VerticalLayout implements
    ...
    public KPAThreeTab(MshenguMain app) {
        main = app;
        kpi = getKpi();
    ...
    private KPI getKpi() {
        KPI kpi = KPIFacade.getKPIService().findByTab("three");
        if (kpi != null) {
            return kpi;
        } else {
            List<KPIItem> items = getItems();
            KPI newkpi = new KPI.Builder("Fuel Management")
                    .tab("three")
                    .items(items)
                    .build();
    ...
    private KPIItem addKPIItems(int i) {
        ...
        int number = i + 1;
        if (i == 0) {
            kPIItem = new KPIItem.Builder("Fuel Efficiency")
            ...
            kPIItem = new KPIItem.Builder("Fuel Spend")
                    .detailedDescription("empty")
                    .measureType("B")
                    .uom("R")
                    .kpiNumber(number)
