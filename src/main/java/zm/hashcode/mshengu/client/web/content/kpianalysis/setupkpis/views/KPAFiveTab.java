 ...
 * @author Luckbliss
 */
public class KPAFiveTab extends VerticalLayout implements
    ...
    public KPAFiveTab(MshenguMain app) {
        main = app;
        kpi = getKpi();
    ...
    private KPA getKpi() {
        KPA kpi = KPIFacade.getKPIService().findByTab("five");
        if (kpi != null) {
            return kpi;
        } else {
            List<KPIItem> items = getItems();
            KPA newkpi = new KPA.Builder("Chemical Usage")
                    .tab("five")
                    .items(items)
                    .build();
