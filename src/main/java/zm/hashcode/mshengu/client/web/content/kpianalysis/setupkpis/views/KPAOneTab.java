 ...
 * @author Luckbliss
 */
public class KPAOneTab extends VerticalLayout implements
    ...
    public KPAOneTab(MshenguMain app) {
        main = app;
        kpi = getKpi();
    ...
    private KPI getKpi() {
        KPI k = KPIFacade.getKPIService().findByTab("one");
        if (k != null) {
            return k;
        } else {
            List<KPIItem> items = getItems();
            KPI newkpi = new KPI.Builder("Field Services (Contract)")
                    .tab("one")
                    .items(items)
                    .build();
    ...
    private KPIItem addKPIItems(int i) {
        ...
        if (i == 0) {
            ...
            kPIItem = new KPIItem.Builder("Unit Deployment")
                    .detailedDescription("empty")
                    .measureType("A")
                    .uom("#")
                    .kpiNumber(number)
