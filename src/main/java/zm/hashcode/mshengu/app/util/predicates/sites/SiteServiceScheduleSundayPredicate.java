 ...
 * @author boniface
 */
public class SiteServiceScheduleSundayPredicate implements Predicate<Site> {
    ...
    private boolean getLatestServiceContract(Site site) {
        if (site != null) {
            return isSundayVisitDay(site.getLastSiteServiceContractLifeCycle());
        }
        return false;
    }

    private boolean isSundayVisitDay(SiteServiceContractLifeCycle siteServiceContractLifeCycle) {
        if (siteServiceContractLifeCycle != null) {
            return siteServiceContractLifeCycle.isSunday();
        }
        return false;
    }
}
