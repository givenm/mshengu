...
public class UIValidatorHelper implements Serializable{
    ...
    public static Validator postalCodeValidation(){
        return new RegexpValidator("[1-9][0-9]{3}", "Postal code must be digits only"); // Postal code that must be digits).
    }
    
    public static Validator phoneNumberValidation(){
        return new RegexpValidator("[1-9][0-9]{9}", "Phone number must be digits only"); // Postal code that must be digits).
    }
    
    public static Validator mobileNumberValidation(){
        return new RegexpValidator("[1-9][0-9]{9}", "Mobile number must be 10 digits only"); // Postal code that must be digits).
    }
    ...
    public static TextField setRequiredTextField(TextField field, String labelName){
        ...
        field.setRequiredError(labelName + " must be filled in!");
        return field;
    }
