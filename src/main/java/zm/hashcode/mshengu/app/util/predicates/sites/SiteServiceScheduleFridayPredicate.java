 ...
 * @author ferox
 */
public class SiteServiceScheduleFridayPredicate implements Predicate<Site> {
    ...
    private boolean getLatestServiceContract(Site site) {
        if (site != null) {
            return isFridayVisitDay(site.getLastSiteServiceContractLifeCycle());
        }
        return false;
    }

    private boolean isFridayVisitDay(SiteServiceContractLifeCycle siteServiceContractLifeCycle) {
        if (siteServiceContractLifeCycle != null) {
            return siteServiceContractLifeCycle.isFriday();
        }
        return false;
    }
}
